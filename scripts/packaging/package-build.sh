...
sed -i bak -e 's/%IMAGEFILE%/$artifactName_docker_image\.tar/g' scripts/packaging/install-image-contacts.sh
sed -i bak -e 's/%IMAGENAME%/unicef\/$artifactName/g' scripts/packaging/install-image-contacts.sh
sed -i bak -e 's/%IMAGEVERSION%/$artifactCounter/g' scripts/packaging/install-image-contacts.sh